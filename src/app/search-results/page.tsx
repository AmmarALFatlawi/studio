...
export default function SearchResultsPage() {
  ...
  const [error, setError] = useState<string | null>(errorParam ? (errorParam === 'refinement_failed' ? 'Query refinement failed. Showing results for the original query.' : 'An unknown error occurred during refinement.') : null);
  ...
  useEffect(()_ => {
    async function fetchData() {
      ...
      setIsLoading(true);
      setError(errorParam ? (errorParam === 'refinement_failed' ? 'Query refinement failed. Using original query for search.' : 'An unknown error occurred during refinement.') : null); // Reset error if not from param
  ...
  const sortedResults = useMemo(()_ => {
    ...
    switch (sortBy) {
      ...
      case 'study_type':
        ...
        break;
      // 'relevance' is default, assuming results from AI are already relevance-sorted
      case 'relevance':
      default:
        // No client-side sort for relevance, assume API returns sorted.
        break;
    }
    return sorted;
  }, [results, sortBy]);
        ...
        <Card className="w-full shadow-lg rounded-xl mb-8">
          ...
          <CardContent className="space-y-4">
            ...
            {refinedQuery && (
             ...
             {error && !error.startsWith("Failed to fetch search results") && ( // Only show refinement errors here
